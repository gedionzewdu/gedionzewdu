    ...
    let t = () => {
        let e = document.location.href,
            ...
            r = new MutationObserver(() => {
                e !== document.location.href && (e = document.location.href, window.top && (window.top.postMessage({
                    type: "URL_CHANGED",
...
var z = () => {
        ...
        window.fetch = async function(...e) {
            ...
            try {
                let r;
                if (e ? .[1] ? .body) try {
                    typeof e[1].body == "string" ? r = e[1].body : e[1].body instanceof FormData ? r = "FormData: " + Array.from(e[1].body.entries()).map(([d, a]) => `${d}=${a}`).join("&") : e[1].body instanceof URLSearchParams ? r = e[1].body.toString() : r = JSON.stringify(e[1].body)
                } catch {
    ...
    P = (() => {
        ...
        return () => {
            ...
            z();
            let o = a => s.has(a) ? !0 : (s.add(a), setTimeout(() => s.delete(a), 5e3), !1),
                d = a => {
                    let T = r(a);
                    if (o(T)) return;
                    let i = e(a);
            ...
            window.addEventListener("error", d), window.addEventListener("unhandledrejection", a => {
...
function A(t, e = {}, s = new WeakMap, r = "root") {
    ...
    if (r.split(".").length > o.maxDepth) return new y("MaxDepthReached", `[Max depth of ${o.maxDepth} reached]`);
    if (t === void 0) return new y("undefined", "undefined");
    if (t === null) return null;
    if (typeof t == "string") return t.length > o.maxStringLength ? new y("String", `${t.slice(0,o.maxStringLength)}... [${t.length-o.maxStringLength} more characters]`) : t;
    if (typeof t == "number") return Number.isNaN(t) ? new y("Number", "NaN") : Number.isFinite(t) ? t : new y("Number", t > 0 ? "Infinity" : "-Infinity");
    if (typeof t == "boolean") return t;
    if (typeof t == "bigint") return new y("BigInt", t.toString());
    if (typeof t == "symbol") return new y("Symbol", t.toString());
    if (typeof t == "function") return new y("Function", {
    ...
    if (t instanceof Error) {
        ...
        for (let E of Object.getOwnPropertyNames(t)) i[E] || (i[E] = A(t[E], o, s, `${r}.${E}`));
        return new y("Error", i)
    }
    if (t instanceof Date) return new y("Date", {
        iso: t.toISOString(),
        value: t.valueOf(),
        local: t.toString()
    });
    if (t instanceof RegExp) return new y("RegExp", {
        source: t.source,
        flags: t.flags,
        string: t.toString()
    });
    if (t instanceof Promise) return new y("Promise", "[Promise]");
    if (t instanceof WeakMap || t instanceof WeakSet) return new y(t.constructor.name, "[" + t.constructor.name + "]");
    if (t instanceof Set) {
        let i = Array.from(t);
    ...
    if (Array.isArray(t)) return t.length > o.maxArrayLength ? t.slice(0, o.maxArrayLength).map((i, E) => A(i, o, s, `${r}[${E}]`)).concat([`... ${t.length-o.maxArrayLength} more items`]) : t.map((i, E) => A(i, o, s, `${r}[${E}]`));
    let d = {},
    ...
    }), a.length > o.maxObjectKeys && (T = a.length - o.maxObjectKeys, d["..."] = `${T} more properties`), d
}
var Q = {
        log: console.log,
        warn: console.warn,
...
var ee = t => {
        let e = s => {
            let o = {
                type: "node",
                children: [],
                attrs: [...s.attributes].reduce((d, a) => (d[a.name] = a.value, d), {}),
                tagName: s.tagName,
                data: R(s)
            };
...
var F = () => {
    window.addEventListener("keydown", t => {
        let e = [];
        t.metaKey && e.push("Meta"), t.ctrlKey && e.push("Ctrl"), t.altKey && e.push("Alt"), t.shiftKey && e.push("Shift");
        let s = t.key !== "Meta" && t.key !== "Control" && t.key !== "Alt" && t.key !== "Shift" ? t.key : "",
            r = [...e, s].filter(Boolean).join("+");
        ["Meta+z", "Meta+Backspace", "Meta+d"].includes(r) && t.preventDefault(), r && f({
            type: "KEYBIND",
            payload: {
                compositeKey: r,
                rawEvent: {
...
var _ = t => t.hasAttribute("data-lov-id") || t.hasAttribute("data-component-path"),
    U = t => {
        if (!t) return {};
        let [e, s, r] = t.split(":");
        return {
    ...
    R = t => {
        let e = t.getAttribute("data-lov-id") || "",
            ...
            T = Array.from(t.children).filter(i => _(i) && L(i).filePath !== s).filter((i, E, b) => E === b.findIndex(O => L(O).filePath === L(i).filePath)).map(i => ({
    ...
    G = () => {
        ...
        F();
        let r = () => {
                e.tooltip = document.createElement("div"), e.tooltip.className = "gpt-selector-tooltip", e.tooltip.setAttribute("role", "tooltip"), document.body.appendChild(e.tooltip);
      ...
      `, document.head.appendChild(n)
            ...
            o = n => {
                if (!(!e.tooltip || !n)) try {
                    ...
                    if (e.tooltip.style.maxWidth = `${c.MAX_TOOLTIP_WIDTH}px`, l) e.tooltip.style.left = c.FULL_WIDTH_TOOLTIP_OFFSET, e.tooltip.style.top = c.FULL_WIDTH_TOOLTIP_OFFSET;
                    else {
                        let p = Math.max(0, u.top - c.TOOLTIP_OFFSET);
                        e.tooltip.style.left = `${Math.max(0,u.left)}px`, e.tooltip.style.top = `${p}px`
                    }
            ...
            d = n => {
                ...
                n.setAttribute(c.HOVERED_ATTR, "true"), u && n.setAttribute("data-full-width", "true")
            },
            a = n => {
                n.removeAttribute(c.HOVERED_ATTR), n.removeAttribute("data-full-width"), n.style.cursor = ""
            },
            T = n => {
            ...
            i = s(n => {
                if (!e.isActive || !_(n.target) || n.target.tagName.toLowerCase() === "html" || T(n.target)) return;
                e.hoveredElement && w(L(e.hoveredElement)).forEach(l => {
                    l.classList.contains("gpt-selected-element") || a(l)
                }), e.hoveredElement = n.target, (e.hoveredElement && w(L(e.hoveredElement))) ? .forEach(g => {
                    g.classList.contains("gpt-selected-element") || d(g)
                }), o(e.hoveredElement), e.tooltip && (e.tooltip.style.display = "block", e.tooltip.style.opacity = "1")
            }, c.DEBOUNCE_DELAY),
            E = s(() => {
                e.isActive && (e.hoveredElement && ((e.hoveredElement && w(L(e.hoveredElement))) ? .forEach(u => {
                    u.removeAttribute(c.HOVERED_ATTR), u.hasAttribute(c.SELECTED_ATTR) || a(u)
                }), e.hoveredElement = null), b())
            }, c.DEBOUNCE_DELAY),
            b = () => {
                e.tooltip && (e.tooltip.style.opacity = "0", e.tooltip.style.display = "none")
            },
            O = () => {
                e.scrollTimeout && clearTimeout(e.scrollTimeout), b(), e.hoveredElement && !e.hoveredElement.classList.contains("gpt-selected-element") && a(e.hoveredElement), e.scrollTimeout = setTimeout(() => {
            ...
            j = () => {
                document.addEventListener("mouseover", i), document.addEventListener("mouseout", E), document.addEventListener("click", x, !0), document.addEventListener("dblclick", q, !0), window.addEventListener("scroll", O, {
                    passive: !0
      ...
      `, document.head.appendChild(n), e.styleElement = n, document.addEventListener("click", S, !0), document.addEventListener("submit", S, !0), document.addEventListener("touchstart", S, !0), document.addEventListener("touchend", S, !0)
            },
            v = () => {
                document.removeEventListener("mouseover", i), document.removeEventListener("mouseout", E), document.removeEventListener("click", x), window.removeEventListener("scroll", O), document.removeEventListener("mousedown", D, !0), document.removeEventListener("click", S, !0), document.removeEventListener("submit", S, !0), document.removeEventListener("touchstart", S, !0), document.removeEventListener("touchend", S, !0), e.styleElement && (e.styleElement.remove(), e.styleElement = null), document.body.style.cursor = "", document.body.style.userSelect = "", document.body.style.msUserSelect = "", document.body.style.mozUserSelect = "", e.hoveredElement && (e.hoveredElement.hasAttribute(c.SELECTED_ATTR) || a(e.hoveredElement), e.hoveredElement = null), b()
            },
            w = n => {
            ...
            B = n => {
                try {
                    if (!n ? .origin || !n ? .data ? .type || !c.ALLOWED_ORIGINS.includes(n.origin)) return;
                    switch (n.data.type) {
                        case "TOGGLE_SELECTOR":
                            let u = !!n.data.payload;
                            e.isActive !== u && (e.isActive = u, e.isActive ? (j(), C().then(() => {
                                document.querySelectorAll("button[disabled]").forEach(l => {
                                    l.removeAttribute("disabled"), l.setAttribute("data-lov-disabled", "")
                                })
                            })) : (v(), document.querySelectorAll("[data-lov-disabled]").forEach(p => {
                                ...
                                p.hasAttribute(c.SELECTED_ATTR) || (a(p), p instanceof HTMLElement && (p.style.cursor = ""))
                            }), e.reset()));
                            break;
                        case "UPDATE_SELECTED_ELEMENTS":
                            ...
                            document.querySelectorAll(`[${c.SELECTED_ATTR}], [${c.HOVERED_ATTR}]`).forEach(l => {
                                l.removeAttribute(c.SELECTED_ATTR), l.removeAttribute(c.HOVERED_ATTR), l.removeAttribute("data-full-width")
                                ...
                                w({
                                    ...
                                    m.setAttribute(c.SELECTED_ATTR, "true"), Math.abs(m.getBoundingClientRect().width - window.innerWidth) < 5 && m.setAttribute("data-full-width", "true")
                                })
                            });
                            break;
                        ...
                        case "DUPLICATE_ELEMENT_REQUESTED":
                            {
                                let {
                                    ...
                                    let h = m.cloneNode(!0);
                                    h.setAttribute("data-lov-id", "x"), h.setAttribute("data-lov-tmp", "true"), m.parentElement ? .appendChild(h)
                                });
                                break
                            }
                        case "SET_STYLESHEET":
                            {
                                ...
                                if (p) p.innerHTML = l;
                                    ...
                                    h.id = c.OVERRIDE_STYLESHEET_ID, h.innerHTML = l, m.appendChild(h)
                                }
                                break
                            }
                        case "EDIT_TEXT_REQUESTED":
                            {
                                let {
                                    ...
                                    let h = () => {
                                        ...
                                        N = () => {
                                            m.removeAttribute("contenteditable"), m.removeEventListener("input", h), m.removeEventListener("blur", N)
                                        };
                                    m.addEventListener("input", h), m.addEventListener("blur", N)
                                });
                                break
                            }
                        case "HOVER_ELEMENT_REQUESTED":
                            {
                                let {
                                    id: l
                                } = n.data.payload;document.querySelectorAll(`[${c.HOVERED_ATTR}]`).forEach(m => {
                        ...
                        case "GET_PARENT_ELEMENT":
                            {
                                ...
                                h = w({
                                    filePath: l.path,
                                    lineNumber: l.line
                                })[0].parentElement;!h || h.id === "root" || ["HTML", "BODY"].includes(h.tagName) ? f({
                                    type: "PARENT_ELEMENT",
                                    payload: null
                                }) : f({
                                    type: "PARENT_ELEMENT",
        ...
        (() => {
            try {
                r(), window.addEventListener("message", B), document.addEventListener("mousemove", V), f({
        ...
        let x = n => {
                if (e.isActive && !(!_(n.target) || n.target.tagName.toLowerCase() === "html" || T(n.target)) && (n.preventDefault(), n.stopPropagation(), e.hoveredElement)) {
                    let u = R(e.hoveredElement);
                    e.hoveredElement.setAttribute(c.SELECTED_ATTR, "true"), Math.abs(e.hoveredElement.getBoundingClientRect().width - window.innerWidth) < 5 && e.hoveredElement.setAttribute("data-full-width", "true"), f({
...
var K = () => {
    let t = e => {
        !e ? .origin || !e ? .data ? .type || !c.ALLOWED_ORIGINS.includes(e.origin) || e.data.type === "NAVIGATE" && (e.data.direction === "back" ? window.history.back() : e.data.direction === "forward" && window.history.forward())
    };
